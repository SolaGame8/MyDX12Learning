...
public:
    ...
    bool CreateShader(int idx);                 //シェーダー
    void CompileShader(const wchar_t* path, const char* entry, const char* target, ID3DBlob** outBlob);
...
private:
    ...
    vector <ComPtr<ID3DBlob>> hullShader;
    vector <ComPtr<ID3DBlob>> domainShader;
    ...
    vector <ComPtr<ID3D12PipelineState>> pipelineState_Grid;     // パイプラインステートGlid表示
    ...
    bool flg_pushMouseBtn_L = false;
    bool flg_pushMouseBtn_R = false;
    ...
    float distanceFromTarget = 3.0f; //ターゲット追従の距離（m）
    float distanceFromTarget_adjust = 0.5f; //ターゲット追従の高さ調整（m）
