...
＊＊＊＊このファイルは、作成中ですこのファイルは、作成中です＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊このファイルは、作成中です＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊このファイルは、作成中です＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊このファイルは、作成中です＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊このファイルは、作成中です＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊

# Steam Input VDFの書き方

## コンフィグ
コントローラのタイプごとに設定がリストされ、次にロードの優先順にリストされます
```vdf
"configurations"
	{
		"controller_xboxone"
...
|------|------|
| title | タイトル |
| StickPadGyro | スティック、パッド、ジャイロ |
| AnalogTrigger | アナログトリガー |
| Button | ボタン |
...
| mode | 対象 | 内容 |
|------|------|------|
| joystick_move | スティック / パッド | スティックのXY座標 |
| joystick_camera | スティック / パッド | カメラ操作（視点移動）に最適化（補正）された挙動 |
| flick_stick | スティック | スティックを倒した方向に一瞬で視点を向ける |
| four_buttons | A/B/X/Yボタン /スティック | 4つの独立したボタンとして扱う |
| switches | 背面ボタンなど | 単発、または複数のスイッチとして扱う |
| trigger | L2 / R2 | トリガー押し込み量（アナログ）を検知する |
| dpad | 十字キー・スティック | 4方向のデジタル入力として扱う |
| absolute_mouse | パッド | マウスそのものとして動作させる |
| scrollwheel | パッド・スティック | 円を描くような動きをマウスホイールとして扱う |
| mouse_joystick | パッド・スティック/ A/B/X/Yボタン | スティック入力をマウス信号に変換して出力する |
...
| 起動条件キー | 内容・発動タイミング |
|----------|-------------|
| full_push | ボタンを最後まで押し切った時に発動（押している間、ずっとイベント継続） |
| start_press | ボタンに触れた（押し始めた）瞬間に発動 |
| soft_push | トリガーを「軽く」引いた時に発動 |
| long_press | ボタンを一定時間（標準0.15秒以上）押し続けた時に発動 |
| double_press | 素早く2回連続で押した時に発動 |
| release | ボタンを「離した」瞬間に発動 |
| chord | 別の特定のボタン（コード）を押しながら押した時に発動 |
